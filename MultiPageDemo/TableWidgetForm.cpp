...
TableWidgetForm::~TableWidgetForm()
{
    ...
    {
        disconnect(PagingWidget_, &PagingWidget::SignalCurrentPageChanged,
                this, &TableWidgetForm::SlotCurrentPageChanged);
...
void TableWidgetForm::UpdateRows(UserData *data, int row)
{
    if(!data || row < 0)
        return;
    int colindex = 0;
    this->ui->tableWidget->setItem(row, colindex++,
                                   new QTableWidgetItem(QString::fromStdString(data->RID)));
    this->ui->tableWidget->setItem(row, colindex++,
                                   new QTableWidgetItem(QString::fromStdString(data->UserID)));
    this->ui->tableWidget->setItem(row, colindex++,
                                   new QTableWidgetItem(QString::fromStdString(data->UserName)));
}
...
void TableWidgetForm::ClearData()
{
